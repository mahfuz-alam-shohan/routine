...
// .wrangler/tmp/bundle-h83UVb/checked-fetch.js
...
// .wrangler/tmp/bundle-h83UVb/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-h83UVb/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
  ...
  if (url.pathname === "/school/teachers") {
    if (request.method === "POST") {
      ...
      if (body.full_name && body.email && body.phone_digits) {
        ...
        await env.DB.prepare("INSERT INTO profiles_teacher (school_id, full_name, subject, email, phone) VALUES (?, ?, ?, ?, ?)").bind(school.id, body.full_name, "", body.email, `880-${cleanPhone}`).run();
        return jsonResponse({ success: true });
      }
      if (body.action === "assign_subjects") {
        ...
        if (body.main_subject) {
          await env.DB.prepare("INSERT INTO teacher_subjects (school_id, teacher_id, subject_id, is_primary) VALUES (?, ?, ?, 1)").bind(school.id, body.teacher_id, body.main_subject, 1).run();
        }
        if (body.additional_subjects && Array.isArray(body.additional_subjects)) {
          for (const subjectId of body.additional_subjects) {
            await env.DB.prepare("INSERT INTO teacher_subjects (school_id, teacher_id, subject_id, is_primary) VALUES (?, ?, ?, 0)").bind(school.id, body.teacher_id, subjectId, 0).run();
          }
        }
        return jsonResponse({ success: true });
      }
