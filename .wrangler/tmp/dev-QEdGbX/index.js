...
// .wrangler/tmp/bundle-E7OHHr/checked-fetch.js
...
// .wrangler/tmp/bundle-E7OHHr/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-E7OHHr/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var DEFINED_SCHEMA = {
  ...
  class_subjects: {
    ...
    // Total classes per week (when fixed is true)
    min_classes: "INTEGER",
    // Minimum classes per week (when fixed is false)
    max_classes: "INTEGER",
    // Maximum classes per week (when fixed is false)
    is_fixed: "BOOLEAN DEFAULT 1",
    // Whether classes_per_week is fixed or flexible
    "FOREIGN KEY(school_id)": "REFERENCES profiles_institution(id)",
    "FOREIGN KEY(class_id)": "REFERENCES academic_classes(id)",
    "FOREIGN KEY(subject_id)": "REFERENCES academic_subjects(id)"
  },
  ...
  teacher_assignments: {
    ...
    // 0 = manual assignment, 1 = auto-assign by system
    created_at: "TIMESTAMP DEFAULT CURRENT_TIMESTAMP",
    "FOREIGN KEY(school_id)": "REFERENCES profiles_institution(id)",
    "FOREIGN KEY(class_id)": "REFERENCES academic_classes(id)",
    "FOREIGN KEY(group_id)": "REFERENCES class_groups(id)",
...
// src/ui/institute/teachers_list.js
init_checked_fetch();
init_modules_watch_stub();
...
function SubjectsPageHTML(school, subjects = [], classes = [], groups = [], sections = [], classSubjects = [], groupSubjects = [], scheduleConfig = {}) {
  ...
  function calculateSectionCapacity(classId, groupId) {
    let totalClasses = 0;
    const classSubjects2 = subjectsByClass[classId] || [];
    totalClasses += classSubjects2.reduce((sum, s) => {
      if (s.is_fixed) {
    ...
    if (groupId) {
      const groupSubjects2 = subjectsByGroup[groupId] || [];
      totalClasses += groupSubjects2.reduce((sum, s) => {
        if (s.is_fixed) {
          return sum + (s.classes_per_week || 0);
        } else {
          return sum + (s.max_classes || 0);
                                    ...
                                    <div class="flex flex-col md:flex-row md:items-center md:justify-between bg-blue-50 p-2 rounded subject-item">
                                        <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                                            ...
                                            <div class="flex flex-wrap gap-2 text-xs">
                                                ${cs.is_fixed ? `<span class="text-gray-600">${cs.classes_per_week} classes/week (fixed)</span>` : `<span class="text-gray-600">${cs.min_classes}-${cs.max_classes} classes/week (flexible)</span>`}
                                            </div>
                                        </div>
                                        ...
                                        ${groupSubjects2.map((gs) => `
                                            <div class="flex flex-col md:flex-row md:items-center md:justify-between bg-purple-50 p-2 rounded subject-item">
                                                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                                                    ...
                                                    <div class="flex flex-wrap gap-2 text-xs">
                                                        ${gs.is_fixed ? `<span class="text-gray-600">${gs.classes_per_week} classes/week (fixed)</span>` : `<span class="text-gray-600">${gs.min_classes}-${gs.max_classes} classes/week (flexible)</span>`}
                                                    </div>
                                                </div>
  ...
  return `
                  ...
                  <div class="space-y-2 border-t pt-4">
                      ...
                      <div class="flex gap-2">
                          <button type="button" onclick="forceDeleteSubject()" 
                              ...
                              \u{1F5D1}\uFE0F Force Delete (Remove from All)
                          </button>
      ...
      <script>
        ...
        function showDeleteReviewModal(subjectId, usageData) {
            ...
            if (usageData.classes && usageData.classes.length > 0) {
                usageHTML += '<div class="bg-blue-50 p-3 rounded border border-blue-200"><strong class="text-blue-800">\u{1F4DA} Used in Classes:</strong><ul class="mt-2 space-y-1">';
                usageData.classes.forEach(cls => {
            ...
            if (usageData.groups && usageData.groups.length > 0) {
                usageHTML += '<div class="bg-purple-50 p-3 rounded border border-purple-200"><strong class="text-purple-800">\u{1F3AF} Used in Groups:</strong><ul class="mt-2 space-y-1">';
                usageData.groups.forEach(group => {
            ...
            if (usageData.teachers && usageData.teachers.length > 0) {
                usageHTML += '<div class="bg-green-50 p-3 rounded border border-green-200"><strong class="text-green-800">\u{1F468}\u200D\u{1F3EB} Assigned to Teachers:</strong><ul class="mt-2 space-y-1">';
...
// src/ui/institute/teachers_assignment.js
init_checked_fetch();
init_modules_watch_stub();
...
function TeachersAssignmentPageHTML(school, classes = [], groups = [], sections = [], subjects = [], classSubjects = [], groupSubjects = [], teachers = [], teacherSubjects = [], teacherAssignments = []) {
  const groupsByClass = {};
  groups.forEach((g) => {
  ...
  function findAssignments(classId, groupId, sectionId, subjectId) {
    return teacherAssignments.filter(
      (ta) => ta.class_id === classId && (ta.group_id === groupId || ta.group_id === null && groupId === null) && (ta.section_id === sectionId || ta.section_id === null && sectionId === null) && ta.subject_id === subjectId
    );
  }
  __name(findAssignments, "findAssignments");
  let tableHTML = "";
    ...
    if (classSubjectsList.length > 0) {
      tableHTML += `
                <tr class="bg-gray-50 font-semibold">
                    <td class="p-3 border" colspan="4">${escapeHtml2(cls.class_name)}</td>
        ...
        if (classSections.length === 0) {
          ...
          tableHTML += `
                        <tr class="border-b">
                            ...
                            <td class="p-3 border"></td>
                            <td class="p-3 border"></td>
                            <td class="p-3 border">${escapeHtml2(subject.subject_name)}</td>
                            <td class="p-3 border">
                                ${existingAssignments.length === 0 ? `
                                        <div class="flex items-center gap-2">
                                            <span class="text-purple-700 font-medium bg-purple-100 px-2 py-1 rounded">\u{1F916} Auto</span>
                                    ...
                                    <div class="space-y-1">
                                        ${existingAssignments.map((assignment) => `
                                            <div class="flex items-center gap-2">
                                                ${assignment.is_auto ? `<span class="text-purple-700 font-medium bg-purple-100 px-2 py-1 rounded">\u{1F916} Auto</span>` : `<span class="text-green-700 font-medium">${escapeHtml2(assignment.teacher_name || "Unknown")}</span>`}
                                                <button type="button" onclick="replaceTeacher(${assignment.id}, '${assignment.is_auto ? "Auto" : escapeHtml2(assignment.teacher_name)}', 'class-${cls.id}-${subject.id}')" 
                                                        class="bg-orange-600 text-white px-2 py-1 text-xs">
                                                    Replace
                                                </button>
                                                <button type="button" onclick="removeTeacher(${assignment.id}, '${assignment.is_auto ? "Auto" : escapeHtml2(assignment.teacher_name)}')" 
          ...
          classSections.forEach((section) => {
            ...
            tableHTML += `
                            <tr class="border-b">
                                <td class="p-3 border"></td>
                                <td class="p-3 border"></td>
                                <td class="p-3 border">${escapeHtml2(section.section_name)}</td>
                                <td class="p-3 border">${escapeHtml2(subject.subject_name)}</td>
                                <td class="p-3 border">
                                    ${existingAssignments.length === 0 ? `
                                        <div class="flex items-center gap-2">
                                    ...
                                    ` : `
                                        <div class="space-y-1">
                                            ${existingAssignments.map((assignment) => `
                                                <div class="flex items-center gap-2">
                                                    ${assignment.is_auto ? `<span class="text-purple-700 font-medium bg-purple-100 px-2 py-1 rounded">\u{1F916} Auto</span>` : `<span class="text-green-700 font-medium">${escapeHtml2(assignment.teacher_name || "Unknown")}</span>`}
                                                    <button type="button" onclick="replaceTeacher(${assignment.id}, '${assignment.is_auto ? "Auto" : escapeHtml2(assignment.teacher_name)}', 'section-${section.id}-${subject.id}')" 
                                                            class="bg-orange-600 text-white px-2 py-1 text-xs">
                                                        Replace
                                                    </button>
                                                    <button type="button" onclick="removeTeacher(${assignment.id}, '${assignment.is_auto ? "Auto" : escapeHtml2(assignment.teacher_name)}')" 
    ...
    sortedGroups.forEach((group) => {
      ...
      if (groupSubjectsList.length > 0) {
        if (lastClassId !== cls.id) {
          tableHTML += `
                        <tr class="bg-gray-50 font-semibold">
                            <td class="p-3 border">${escapeHtml2(cls.class_name)}</td>
        ...
        tableHTML += `
                    <tr class="bg-gray-100">
                        <td class="p-3 border"></td>
                        <td class="p-3 border font-medium">${escapeHtml2(group.group_name)}</td>
                                    ...
                                    ` : `
                                        <div class="space-y-1">
                                            ${existingAssignments.map((assignment) => `
                                                <div class="flex items-center gap-2">
                                                    ${assignment.is_auto ? `<span class="text-purple-700 font-medium bg-purple-100 px-2 py-1 rounded">\u{1F916} Auto</span>` : `<span class="text-green-700 font-medium">${escapeHtml2(assignment.teacher_name || "Unknown")}</span>`}
                                                    <button type="button" onclick="replaceTeacher(${assignment.id}, '${assignment.is_auto ? "Auto" : escapeHtml2(assignment.teacher_name)}', 'group-${group.id}-${subject.id}')" 
                                                            class="bg-orange-600 text-white px-2 py-1 text-xs">
                                                        Replace
                                                    </button>
                                                    <button type="button" onclick="removeTeacher(${assignment.id}, '${assignment.is_auto ? "Auto" : escapeHtml2(assignment.teacher_name)}')" 
  ...
  return `
                  ...
                  <form onsubmit="saveAssignment(event)" class="space-y-4">
                      ...
                      <div>
                          ...
                          <select id="teacher-select" required class="w-full border px-3 py-2">
                              ...
                              ${teachers.map((t) => `<option value="${t.id}">${escapeHtml2(t.full_name)}</option>`).join("")}
                          </select>
                      </div>
                      <div class="flex gap-2 pt-2 border-t">
                          <button type="submit" class="bg-blue-600 text-white px-4 py-2">
      ...
      <script>
        ...
        function replaceTeacher(assignmentId, currentTeacherName, assignmentKey) {
            ...
            autoOption.textContent = '\u{1F916} Auto-assign (System will decide)';
...
__name(TeachersAssignmentPageHTML, "TeachersAssignmentPageHTML");
    ...
    if (request.method === "POST") {
      try {
        ...
        if (body.action === "add_class_subject") {
          const existingSubject = await env.DB.prepare("SELECT id FROM class_subjects WHERE school_id = ? AND class_id = ? AND subject_id = ?").bind(body.school_id, body.class_id, body.subject_id).first();
          ...
          if (isFixed) {
            await env.DB.prepare("INSERT INTO class_subjects (school_id, class_id, subject_id, classes_per_week, is_fixed) VALUES (?, ?, ?, ?, ?)").bind(body.school_id, body.class_id, body.subject_id, body.classes_per_week, 1).run();
          } else {
            await env.DB.prepare("INSERT INTO class_subjects (school_id, class_id, subject_id, min_classes, max_classes, is_fixed) VALUES (?, ?, ?, ?, ?, ?)").bind(body.school_id, body.class_id, body.subject_id, body.min_classes, body.max_classes, 0).run();
          }
          await env.DB.prepare(`
        ...
        if (body.action === "add_group_subject") {
          const existingSubject = await env.DB.prepare("SELECT id FROM group_subjects WHERE school_id = ? AND group_id = ? AND subject_id = ?").bind(body.school_id, body.group_id, body.subject_id).first();
          ...
          const existingClassSubject = await env.DB.prepare("SELECT id FROM class_subjects WHERE school_id = ? AND class_id = ? AND subject_id = ?").bind(body.school_id, classInfo.class_id, body.subject_id).first();
          ...
          const isFixed = body.is_fixed === true || body.is_fixed === "true";
          if (isFixed) {
            await env.DB.prepare("INSERT INTO group_subjects (school_id, group_id, subject_id, classes_per_week, is_fixed) VALUES (?, ?, ?, ?, ?)").bind(body.school_id, body.group_id, body.subject_id, body.classes_per_week, 1).run();
          } else {
            await env.DB.prepare("INSERT INTO group_subjects (school_id, group_id, subject_id, min_classes, max_classes, is_fixed) VALUES (?, ?, ?, ?, ?, ?)").bind(body.school_id, body.group_id, body.subject_id, body.min_classes, body.max_classes, 0).run();
          }
          await env.DB.prepare(`
                      INSERT INTO teacher_assignments (school_id, class_id, group_id, section_id, subject_id, teacher_id, is_auto) 
                      VALUES (?, ?, ?, ?, ?, ?, 1)
    ...
    if (request.method === "POST") {
      ...
      if (body.action === "assign_auto") {
        await env.DB.prepare(`
          ...
          body.subject_id,
          null
          // teacher_id is null for auto assignments
        ).run();
        return jsonResponse({ success: true });
      }
