...
// .wrangler/tmp/bundle-ermAyi/checked-fetch.js
...
// .wrangler/tmp/bundle-ermAyi/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-ermAyi/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var DEFINED_SCHEMA = {
  ...
  schedule_config: {
    ...
    // Number of active school days per week
...
init_checked_fetch();
init_modules_watch_stub();
function TeachersPageHTML(teachers = [], allSubjects = []) {
  const formatSub = /* @__PURE__ */ __name((jsonStr) => {
    try {
  ...
  const rows = teachers.map((t) => `
        <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 md:px-6 py-3 md:py-4 whitespace-normal max-w-xs md:max-w-sm">
                <div class="flex flex-wrap gap-1">
                    ${formatSub(t.subject).split(", ").map(
    (s) => `<span class="bg-blue-50 text-blue-700 px-1.5 md:px-2 py-0.5 md:py-1 rounded text-[10px] md:text-xs uppercase font-bold tracking-wide border border-blue-100">${s}</span>`
  ).join("")}
                </div>
            </td>
                  ...
                  ${teachers.map((t) => `
                      <div class="border border-gray-200 rounded-lg p-3">
                          ...
                          <div class="flex flex-wrap gap-1 mt-2">
                              ${formatSub(t.subject).split(", ").map(
    (s) => `<span class="bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded text-[10px] uppercase font-bold tracking-wide border border-blue-100">${s}</span>`
  ).join("")}
                          </div>
                          <div class="text-xs font-mono text-gray-600 mt-2">${t.phone}</div>
                      </div>
             ...
             <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                 ...
                 <div class="text-sm text-gray-600">
                     ...
                     <div class="text-xs text-gray-500 mt-1">
                         (Calculated from master schedule: ${scheduleConfig.active_days || 5} days \xD7 ${scheduleConfig.actualClassPeriodsPerDay || 8} class periods)
                     </div>
                 </div>
             </div>
...
init_checked_fetch();
init_modules_watch_stub();
function SubjectsPageHTML(subjects = [], classes = [], allocations = []) {
  const assignedMap = {};
...
async function handleInstituteRequest(request, env) {
    ...
    if (request.method === "POST") {
      try {
        ...
        if (body.action === "save_schedule") {
          ...
          const batch = slots2.map((s) => stmt.bind(school.id, s.start_time, s.end_time, s.duration, s.label, s.type));
          await env.DB.batch(batch);
          if (slots2.length > 0) {
            await env.DB.prepare("UPDATE schedule_config SET start_time = ? WHERE school_id = ?").bind(slots2[0].start_time, school.id).run();
            const existing = await env.DB.prepare("SELECT id FROM schedule_config WHERE school_id = ?").bind(school.id).first();
            if (!existing) {
              await env.DB.prepare("INSERT INTO schedule_config (school_id, start_time) VALUES (?, ?)").bind(school.id, slots2[0].start_time).run();
            }
          }
        }
        return jsonResponse({ success: true });
  ...
  if (url.pathname === "/school/teachers") {
    if (request.method === "POST") {
      ...
      await env.DB.prepare("INSERT INTO profiles_teacher (school_id, full_name, subject, email, phone) VALUES (?, ?, ?, ?, ?)").bind(school.id, body.full_name, body.subject, body.email, `880-${cleanPhone}`).run();
      return jsonResponse({ success: true });
    }
  ...
  if (url.pathname === "/school/classes") {
    ...
    if (!scheduleConfig) {
      await env.DB.prepare("INSERT INTO schedule_config (school_id, active_days, periods_per_day) VALUES (?, ?, ?)").bind(school.id, 5, 8).run();
